...
class FaceMeshMediapipe:
    def __init__(self):
        ...
        self.config_draw = self.mp_draw.DrawingSpec(color=(255, 127, 0), thickness=1, circle_radius=1)
    ...
    def face_mesh_mediapipe(self, face_image: np.ndarray):
    ...
    def extract_face_mesh_points(self, face_image: np.ndarray, face_info: Any, viz: bool) -> List[List[int]]:
        height, width, _ = face_image.shape
        self.mesh_points = []
        for face_mesh in face_info.multi_face_landmarks:
